...
class _FloatingWindowState extends State<FloatingWindow> {
  ...
  Widget _buildBarrier() {
    return widget.barrier ??
        GestureDetector(
          onTap:
              () =>
                  _controller.onMinimize(widget.showBarrier, widget.onMinimize),
  ...
  Widget _buildContent(Size screenSize, Size windowSize, bool fullscreen) {
    final radius =
        (fullscreen || widget.initialHeight == double.infinity)
            ? BorderRadius.zero
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (bool didPop, void result) {
        if (!didPop) {
          ...
          if (!_controller.manaState.floatWindowMainVisible.value ||
              !widget.showBarrier) {
            _controller.manaState.activePluginPanelVisible.value = false;
              ...
              builder: (context, _) {
                ...
                return Positioned(
                  ...
                  child: Visibility(
                    ...
                    child: _buildContent(
                      _controller.screenSize,
                      _controller.windowSize,
                      fullscreen,
                    ),
                  ),
                );
