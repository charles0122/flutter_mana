...
import 'package:mana_channel_monitor/src/ui/template_ui.dart';
import 'package:rxdart/subjects.dart';

/// 监控页面集合：频道列表、单频道记录、记录详情
...
class _ChannelMonitorPagesState extends State<ChannelMonitorPages> {
  ...
  ChannelDetailPage buildChannelInfoPage() {
...
class _SingleChannelPageState extends State<SingleChannelPage> {
  ...
  Widget build(BuildContext context) {
    return TemplatePageWidget(
      ...
      title: widget.title,
      body: StreamBuilder<List<ChannelRecord>>(
        stream: _publisher,
        builder: (context, snapshot) {
          if (!snapshot.hasData) {
            return const Center(child: CircularProgressIndicator());
          ...
          final List<ChannelRecord> channels =
              snapshot.data as List<ChannelRecord>;
          return ListView.builder(
            padding: EdgeInsets.zero,
            itemCount: channels.length,
            itemBuilder: (context, index) {
