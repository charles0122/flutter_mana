...
///zone的error处理方法
/// * 可以覆盖此方法，然后自处理相关错误
/// * PS. 如果覆盖[zoneErrorHandler],将导致[ManaChannelObserver._errorStreamController]
/// * 、[ChannelObserverWidget]的失效。
ZoneErrorHandler zoneErrorHandler = ManaChannelObserver._defaultErrorHandler;
  ...
  static void customZone(
    ...
    FlutterError.onError = (details) {
      Zone.current.handleUncaughtError(
        details.exception,
        details.stack ?? StackTrace.fromString('empty'),
      );
    };
  }
...
class BinaryMessengerProxy extends BinaryMessenger {
  ...
  void _resolveResult(ChannelModel model, ByteData? result) {
    try {
      if (model.methodCodec != null) {
        model.results = model.methodCodec?.decodeEnvelope(
          result ?? ByteData(1),
        );
  ...
  Future<void> handlePlatformMessage(
    ...
    ServicesBinding.instance.channelBuffers.push(channel, data, (ByteData? d) {
      if (callback != null) {
        callback(d);
      }
    });
