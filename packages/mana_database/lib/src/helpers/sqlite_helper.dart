...
class SqliteHelper implements ManaDatabaseHelper {
  SqliteHelper();
  ...
  Future<List<TableData>> findAllTableData(UMEDatabase umeDatabase) async {
    ...
    if (umeDatabase is SqliteDatabase) {
      var tables = await umeDatabase.db?.rawQuery(
      ...
      for (var tableMap in tables) {
        ...
        // print(tableMap);
        var columnData = await sqliteDatabase.db?.rawQuery(
  ...
  Future<void> updateData(
    ...
    for (var map in updateMaps) {
      await sqliteDatabase.db?.update(
        tableName,
        map,
        where: where,
        whereArgs: whereArgs,
