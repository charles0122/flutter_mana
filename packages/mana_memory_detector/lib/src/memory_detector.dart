...
import 'storage/leaked_info_recorder.dart';
...
mixin DetectTaskRunner on VmServiceDelegate {
  ...
  Future<List<dynamic>> _getPropertyList(Expando expando) async {
    if (await hasVmService) {
      final data = (await _getInstanceByObj(
        expando,
...
mixin VmServiceDelegate {
  ...
  Future<Isolate?> _getMainIsolate() async {
    ...
    if (vm != null) {
      final IsolateRef? ref = vm.isolates?.firstWhere(
  ...
  Future<String?> _getObjectId(dynamic obj) async {
    ...
    Response keyRes = await vms.invoke(
      mainIso.id!,
      detectorLib.id!,
      'generateNewObjKey',
      [],
    ...
    try {
      Response objRes = await vms.invoke(
        mainIso.id!,
        detectorLib.id!,
        'key2Obj',
        [keyRef!.id!],
...
class LeakAnalyzer with VmServiceDelegate {
  ...
  /// 分析单个泄漏节点，构建引用链为 RetainingNode 列表
  /// 1) 通过 vm_service 获取 retaining path（最多 `maxRetainingPath`）
  /// 2) 将每个 RetainingObject 解析为 RetainingNode（含源码/容器/闭包信息）
  /// 3) 返回包含 gcRootType 的 `LeakedInfo`
  /// 在子 isolate 中运行，避免阻塞 UI 线程
  static Future<LeakedInfo?> analyze(RawLeakNode node) async {
    ...
    if (leakedInstance?.id != null) {
      final retainingPath = await _analyzer!._getRetainingPath(
        leakedInstance!.id!,
        maxRetainingPath,
      );
  ...
  static Future<ClosureInfo?> _getClosureInfo(Instance? instance) async {
    if (instance != null && instance.kind == 'Closure') {
      ...
      final info = ClosureInfo(
        closureFunctionName: name,
        closureOwner: owner?.name,
      );
      ...
      if (ref is LibraryRef) {
        ...
        if (info.funLine == null) {
          ...
          if (func?.location?.script?.id != null) {
            Script? script =
                (await _analyzer!._getObjInstanceById(
                      func!.location!.script!.id!,
                    ))
                    as Script?;
            if (script != null && func.location?.tokenPos != null) {
              info.funLine = script.getLineNumberFromTokenPos(
                func.location!.tokenPos!,
              );
              info.funColumn = script.getColumnNumberFromTokenPos(
                func.location!.tokenPos!,
              );
            }
          }
  ...
  static Future<String?> _map2String(RetainingObject retainingObject) async {
    ...
    if (retainingObject.parentMapKey?.id != null) {
      Obj? keyObj = await _analyzer!._getObjInstanceById(
        retainingObject.parentMapKey!.id!,
      );
      if (keyObj?.json != null) {
  ...
  static Future<SourceCodeLocation?> _getSourceCodeLocation(
    ...
    if (parentField != null && clz.name != '_Closure') {
      List? filedAndClass = await _getFieldAndClassByName(
        clz,
        Uri.encodeQueryComponent(parentField),
      );
      if (filedAndClass != null) {
        ...
        if (fieldRef.id != null) {
          ...
          if (field != null) {
            ...
            Script? script =
                (await _analyzer!._getObjInstanceById(
                      field.location!.script!.id!,
                    ))
                    as Script?;
            if (script != null && field.location?.tokenPos != null) {
              int? line = script.getLineNumberFromTokenPos(
                field.location!.tokenPos!,
              );
              int? column = script.getColumnNumberFromTokenPos(
                field.location!.tokenPos!,
              );
              String? codeLine =
                  script.source
  ...
  static Future<LeakedNodeType> _getObjectType(Class? clazz) async {
    ...
    if (clazz.superClass?.id != null) {
      Class? superClass =
          (await _analyzer!._getObjInstanceById(clazz.superClass!.id!))
              as Class?;
...
class FieldRefParser implements ObjectAnalyzeStrategy {
  ...
  Future<RetainingNode?> analyzeObject2Node(
    ...
    if (fieldRef.name != null && clz != null) {
      sourceCodeLocation = await LeakAnalyzer._getSourceCodeLocation(
        fieldRef.name,
        clz,
      );
    }
...
class InstanceRefParser implements ObjectAnalyzeStrategy {
  ...
  Future<RetainingNode?> analyzeObject2Node(
    ...
    if (instanceRef.classRef?.id != null) {
      clz =
          (await analyzer._getObjInstanceById(instanceRef.classRef!.id!))
              as Class?;
    }
