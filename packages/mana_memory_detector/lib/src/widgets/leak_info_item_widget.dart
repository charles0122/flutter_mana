...
/// 泄露路径中的单个节点展示项
/// 展示类名、字段、节点类型，并可附带源码位置或容器索引/键等信息
class MemoryLeakItemWidget extends StatelessWidget {
  const MemoryLeakItemWidget({
    Key? key,
    required this.node,
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        Container(height: 0.8, color: Colors.white70),
      ],
    );
  }
  ...
  Widget _builNoneClosureHeader() {
    return RichText(
      text: TextSpan(
        text: node.clazz,
        children: [
          if (hasField) const TextSpan(text: '.'),
          if (hasField)
            TextSpan(
    ...
    return Column(
      ...
      children: [
        ...
        RichText(
          text: TextSpan(
            text:
                node.closureInfo?.closureOwnerClass == null
        ...
        children: [
          Positioned(
            ...
            child: SizedBox(
              width: 30,
              child: Center(
                child: Container(color: lineColor, width: _strokeWidth),
              ),
            ),
          ),
          Container(
  ...
  Widget _buildMoreWidget() {
    return Padding(
      ...
      child: SizedBox(
        width: 30,
        height: 18,
        child: Center(child: Container(color: lineColor, width: _strokeWidth)),
      ),
    );
  }
